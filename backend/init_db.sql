...
-- 2) Rename users -> profiles (if still using the legacy name)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
...
-- 3) Profiles: one actor model with unlockable capabilities
ALTER TABLE IF EXISTS profiles
    ADD COLUMN IF NOT EXISTS major_id INTEGER,
    ADD COLUMN IF NOT EXISTS academic_year_id INTEGER,
    ADD COLUMN IF NOT EXISTS can_tutor BOOLEAN NOT NULL DEFAULT FALSE,
...
BEGIN
    IF EXISTS (
        ...
        EXECUTE $sql$
            ...
            SET
                can_tutor = CASE
                    WHEN role IN ('tutor', 'admin') THEN TRUE
                    ELSE can_tutor
                END,
                is_verified_tutor = CASE
                    WHEN role IN ('tutor', 'admin') THEN TRUE
                    ELSE is_verified_tutor
                END,
                is_admin = CASE
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tutor_application_status') THEN
        CREATE TYPE tutor_application_status AS ENUM ('pending', 'approved', 'rejected');
    END IF;
END $$;

-- 6) Events shape updates
...
BEGIN
    IF EXISTS (
        ...
        EXECUTE 'UPDATE events SET type = COALESCE(type, event_type, ''academic'')';
    ELSE
        EXECUTE 'UPDATE events SET type = COALESCE(type, ''academic'')';
    END IF;
...
ALTER TABLE IF EXISTS events
    ...
    USING (
        CASE
            WHEN LOWER(COALESCE(type, 'academic')) IN ('academic', 'entrepreneurial')
                THEN LOWER(COALESCE(type, 'academic'))
            ELSE 'academic'
        END
    )::event_type,
    ALTER COLUMN delivery_mode TYPE delivery_mode
    USING (
        CASE
            WHEN LOWER(COALESCE(delivery_mode, 'onsite')) = 'online' THEN 'online'
            ELSE 'onsite'
        END
    )::delivery_mode,
    ALTER COLUMN type SET DEFAULT 'academic',
    ALTER COLUMN delivery_mode SET DEFAULT 'onsite',
...
-- 7) Event participants table (from event_registrations if present)
...
CREATE TABLE IF NOT EXISTS tutor_applications (
    ...
    professor_id UUID NULL REFERENCES profiles(id),
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    CONSTRAINT uq_tutor_application_user_subject UNIQUE (user_id, subject_id)
);
...
-- 9) Recordings uploaded after events
...
CREATE INDEX IF NOT EXISTS idx_recordings_event_id
    ON recordings (event_id);

-- 10) Reviews now tied to events
